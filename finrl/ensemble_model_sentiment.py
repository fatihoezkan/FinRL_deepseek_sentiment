...
# Calculate weights based on risk-adjusted Sharpe ratios
...
# Adjust weights based on current risk level
current_risk = risk_scores['risk_score'].iloc[-1]
if current_risk > 0.7:  # High risk
    # Give more weight to conservative models (e.g., A2C, PPO)
    weights = base_weights.copy()
    weights[['A2C', 'PPO']] *= 1.2
    weights[['SAC', 'TD3']] *= 0.8
elif current_risk < 0.3:  # Low risk
    # Give more weight to aggressive models (e.g., SAC, TD3)
    weights = base_weights.copy()
    weights[['SAC', 'TD3']] *= 1.2
    weights[['A2C', 'PPO']] *= 0.8
else:
...
# Save to CSV
df_account_value_ensemble.to_csv('result_with_ensemble.csv')
